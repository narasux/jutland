...
func (d *Drawer) Draw(screen *ebiten.Image, misState *state.MissionState) {
	d.drawCameraAndConsole(screen, misState)
...
// 绘制背景（相机视野 + 控制台）
func (d *Drawer) drawCameraAndConsole(screen *ebiten.Image, ms *state.MissionState) {
...
func (d *Drawer) drawArrowOnMapWhenHover(screen *ebiten.Image, ms *state.MissionState) {
	...
	case action.HoverScreenLeft:
		posX, posY, rotate = padding, ms.Layout.Camera.Height/2, 180
	case action.HoverScreenRight:
		posX, posY, rotate = ms.Layout.Camera.Width-imgW-padding, ms.Layout.Camera.Height/2, 0
	case action.HoverScreenTop:
		posX, posY, rotate = ms.Layout.Camera.Width/2, padding, 270
	case action.HoverScreenBottom:
		posX, posY, rotate = ms.Layout.Camera.Width/2, ms.Layout.Camera.Height-imgH-padding, 90
	case action.HoverScreenTopLeft:
		posX, posY, rotate = padding, padding, 225
	case action.HoverScreenTopRight:
		posX, posY, rotate = ms.Layout.Camera.Width-imgW-padding, padding, 315
	case action.HoverScreenBottomLeft:
		posX, posY, rotate = padding, ms.Layout.Camera.Height-imgH-padding, 135
	case action.HoverScreenBottomRight:
		posX, posY, rotate = ms.Layout.Camera.Width-imgW-padding, ms.Layout.Camera.Height-imgH-padding, 45
	default:
		return
	}
...
func (d *Drawer) drawBattleShips(screen *ebiten.Image, ms *state.MissionState) {
	for _, ship := range ms.Ships {
		// 只有在屏幕中的才渲染
		if ship.Pos.MX < ms.Camera.Pos.MX ||
			ship.Pos.MX > ms.Camera.Pos.MX+ms.Camera.Width ||
			ship.Pos.MY < ms.Camera.Pos.MY ||
			ship.Pos.MY > ms.Camera.Pos.MY+ms.Camera.Height {
			continue
		}
		...
		opts.GeoM.Translate(
			(ship.Pos.RX-float64(ms.Camera.Pos.MX))*mapblock.BlockSize,
			(ship.Pos.RY-float64(ms.Camera.Pos.MY))*mapblock.BlockSize,
		)
