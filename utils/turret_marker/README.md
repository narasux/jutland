...
3. 舰体画布占满页面宽度，标记列表和射界/导出面板排列在画布下方。
4. 工具会根据 Alpha 边界自动识别舰体首尾端点；如识别不准，点击“手动校准”，分别点击两个端点，顺序不限。
5. 选择武器类型，然后点击炮塔位置。
6. 列表显示每个标记的 `posPercent`，可删除错误标记。
7. 点击“复制项目 JSON”导出包含类型、左右舷、像素坐标、两位小数 `posPercent`、射界和数量校验的结构化结果。
8. 选中某个标记时，地图会以半透明圆周扇区显示其射界，并延伸至当前预览画布边缘；绿色表示右舷，蓝色表示左舷。
...
“对称吸附”默认开启。连续点击同一类型的两个对称炮塔时，如果两次换算结果的差值小于吸附阈值，第二次会复用第一次的精确 `posPercent`，避免手工点击误差造成同组炮塔数值不一致。若找到对侧同型炮位，射界会按舰体艏艉轴镜像复制，即角度 `a -> 360° - a` 并交换左右舷；例如右舷 `[0, 90]` 会生成左舷 `[270, 360]`。

需要独立记录两个相近位置时，可以取消勾选，或调低吸附阈值。

## 射界参考

射界面板会根据武器类型和当前位置给出常见射界推荐，例如主炮前向、后向和中段射界，也可以人工修正。修正后的角度会写入“复制项目 JSON”的导出结果，并实时显示在地图扇区上。

每个角度输入框下方提供常用角度按钮。右舷使用 `0°`、`30°`、`45°`、`90°`、`135°`、`150°`、`180°`；左舷使用 `180°`、`210°`、`225°`、`270°`、`315°`、`330°`、`360°`。选中标记后点击按钮即可填入对应角度，无需键盘输入。
