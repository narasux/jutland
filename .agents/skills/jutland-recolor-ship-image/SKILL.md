...
- 俯视图里同批主炮塔可能一部分是浅色顶、一部分是深灰顶；先统计深色顶的精确颜色和连通组件包围盒，再逐座框选处理。
- 常见配色：深灰顶 `125,125,125`，其斜面/装饰线 `105,105,105`；浅色顶 `228,228,228`，细节线 `193,193,193`。用 `-mode pixels` 在每座深色顶的 `-include-box` 内把 `125->228`、`105->193`，即可与浅色顶统一。
